...
import finalproject.levelmodel.peamove;
import finalproject.levelmodel.wallmove;
import javafx.scene.media.AudioClip;

public class level2test extends JFrame implements ppp, ActionListener {
	...
	public level2test() {
		...
		timer0 = new Timer();
		timer0.schedule(new zt0(), 14000, 13000);
		timer3 = new Timer();
		timer3.schedule(new zt3(), 16000, 14000);
		timer4 = new Timer();
		timer4.schedule(new zt4(), 1000, 12000);
		timer5 = new Timer();
		timer5.schedule(new zt5(), 24000, 12000);
		timer2 = new Timer();
		timer2.schedule(new zt2(), 7000, 15000);
		timerre = new Timer();
	...
	class jp2 extends JPanel {
		...
		public void paint(Graphics g) {
			...
			if (zombiedienum > 17 && z0.isEmpty() && z2.isEmpty() && z3.isEmpty() && z4.isEmpty() && z5.isEmpty()
	...
	public class zt0 extends TimerTask {// 第一道殭屍
		...
		public void run() {
			...
			if (zombienum > 12) {
				...
				trizt0.schedule(new trizt0(), 13000);
				cancel();
			} else {
				z0.add(new zombie());
				zombienum++;
	...
	class zt5 extends TimerTask {// 第五道殭屍
		...
		public void run() {
			...
			if (zombienum > 12) {
				...
				trizt5.schedule(new trizt5(), 16000);
				cancel();
			} else {
				z5.add(new zombie());
				zombienum++;
	...
	class zt3 extends TimerTask {// 第三道殭屍
		...
		public void run() {
			...
			if (zombienum > 12) {
				...
				trizt3.schedule(new trizt3(), 15000);
				cancel();
			} else {
				z3.add(new zombie());
				zombienum++;
	...
	class zt2 extends TimerTask {// 第二道殭屍
		...
		public void run() {
			...
			if (zombienum > 12) {
				...
				trizt2.schedule(new trizt2(), 10000);
				cancel();
			} else {
				z2.add(new zombie());
				zombienum++;
	...
	class zt4 extends TimerTask {// 第四道殭屍
		...
		public void run() {
			...
			if (zombienum > 12) {
				...
				trizt4.schedule(new trizt4(), 5000);
				cancel();
			} else {
				z4.add(new zombie());
				zombienum++;
